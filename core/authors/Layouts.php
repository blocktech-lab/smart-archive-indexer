...
class Layouts implements iLayouts {
	...
	protected function style( $id, $args = array() ) : string {
		...
		foreach ( $supported as $key ) {
			if ( isset( $args[ 'var-' . $key ] ) && ! empty( $args[ 'var-' . $key ] ) ) {
				$vars[] = '--smartarchiveindexer-authors-' . $key . ': ' . $args[ 'var-' . $key ] . ';';
			}
		}
	...
	protected function avatar( $info, $args = array() ) : string {
		...
		if ( $args['avatar'] > 0 ) {
			$avatar = apply_filters( 'smartarchiveindexer-authors-avatar', null, $info, $args['avatar'] );
	...
	protected function get_author_link( $post_type, $author_id, $author_nicename ) : string {
		$url = apply_filters( 'smartarchiveindexer-authors-get-author-link-' . $post_type, null, $author_id, $author_nicename );
	...
	protected function basic( $data, $args = array() ) : string {
		$classes = array(
			...
			'smartarchiveindexer-authors-wrapper',
			'smartarchiveindexer-authors-layout-' . $args['layout'],
		);
		...
		$classes[] = 'smartarchiveindexer-authors-columns-' . $columns;

		if ( ! empty( $args['class'] ) ) {
			$classes[] = $args['class'];
		}

		$id = 'smartarchiveindexer-authors-block-' . ( ++ $this->id );

		$render = '<div class="wp-' . $args['_source'] . '-smartarchiveindexer-authors">';
		...
		foreach ( $data as $user_id => $info ) {
			...
			$_class   = 'smartarchiveindexer-authors-author';
			...
			if ( ! empty( $_avatar ) ) {
				...
				$render .= '<a title="' . sprintf( _nx( '%1$s: %2$d Post', '%1$s: %2$d Posts', $_count, 'Author name and posts count', 'smartarchiveindexer' ), $info['name'], $_count ) . '" class="link-avatar" href="' . $_archive . '">' . $_avatar . '</a>';
			}
			...
			/* translators: 1. Author Name, 2. Number of Posts */
			$render .= '<a title="' . sprintf( _nx( '%1$s: %2$d Post', '%1$s: %2$d Posts', $_count, 'Author name and posts count', 'smartarchiveindexer' ), $info['name'], $_count ) . '" class="link-name" href="' . $_archive . '">' . $info['name'] . '</a>';
			$render .= '</div>';
		}
